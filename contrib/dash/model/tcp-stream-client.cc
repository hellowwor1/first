...
void TcpStreamClient::Controller(controllerEvent event) {
  ...
  if (state == downloading) {
    ...
    if (m_currentPlaybackIndex <= m_lastSegmentIndex) {  // 如果还有 segment 未播放
      /*  e_d  */                                        // 状态注释标记，表示下载事件
  ...
  // 如果当前状态是 downloadingPlaying（下载+播放）
    ...
    if (event == downloadFinished) {                // 如果触发事件是下载完成
      ...
      } else if (m_segmentCounter == m_lastSegmentIndex) {  // 如果当前下载最后一段
        /*  e_df */                                         // 下载完成标记
  ...
  // 如果当前状态是 playing（只播放）
    ...
    if (event == irdFinished) {    // 如果延迟下载事件完成
      ...
      Send(m_videoData.segmentSize.at(m_currentRepIndex).at(m_segmentCounter));
               ...
               m_currentPlaybackIndex < m_lastSegmentIndex) {  // 如果播放完成，且还有 segment
      /*  e_pb */                                              // 播放完成标记
      ...
      Simulator::Schedule(MicroSeconds(m_videoData.segmentDuration), &TcpStreamClient::Controller, this, ev);
               ...
               m_currentPlaybackIndex == m_lastSegmentIndex) {  // 如果播放完成，且已经是最后 segment
  ...
  static TypeId tid =
      TypeId("ns3::TcpStreamClient")          // 类的名字，用于 NS3 类型系统
          ...
          .AddAttribute("RemoteAddress", "The destination Address of the outbound packets",
                        AddressValue(),                                        // 默认值为空
                        MakeAddressAccessor(&TcpStreamClient::m_peerAddress),  // 绑定到类成员
                        MakeAddressChecker())                                  // 类型检查

          // 添加 RemotePort 属性，表示客户端要连接的远端端口
          .AddAttribute("RemotePort", "The destination port of the outbound packets",
                        UintegerValue(0),  // 默认值 0
                        MakeUintegerAccessor(&TcpStreamClient::m_peerPort), MakeUintegerChecker<uint16_t>())
          ...
          .AddAttribute("SegmentDuration", "The duration of a segment in microseconds",
                        ...
                        MakeUintegerAccessor(&TcpStreamClient::m_segmentDuration), MakeUintegerChecker<uint64_t>())
          ...
          */
          ...
          .AddAttribute("SegmentSizeFilePath",
                        ...
                        MakeStringAccessor(&TcpStreamClient::m_segmentSizeFilePath), MakeStringChecker())
          ...
          .AddAttribute("VideoSegmentSizeFilePath",
                        ...
                        "containing the segment sizes in bytes",
                        StringValue("bitrates.txt"),  // 默认文件名
                        MakeStringAccessor(&TcpStreamClient::m_videosegmentSizeFilePath), MakeStringChecker())
          ...
          .AddAttribute("AudioSegmentSizeFilePath",
                        ...
                        "containing the segment sizes in bytes",
                        StringValue("bitrates.txt"),  // 默认文件名
                        MakeStringAccessor(&TcpStreamClient::m_audiosegmentSizeFilePath), MakeStringChecker())

          // 添加 SimulationId 属性，用于日志标识当前仿真
          .AddAttribute("SimulationId", "The ID of the current simulation, for logging purposes", UintegerValue(0),
                        MakeUintegerAccessor(&TcpStreamClient::m_simulationId), MakeUintegerChecker<uint32_t>())

          // 添加 NumberOfClients 属性，总客户端数，用于日志
          .AddAttribute("NumberOfClients",
                        "The total number of clients for this simulation, for logging "
                        "purposes",
                        UintegerValue(1), MakeUintegerAccessor(&TcpStreamClient::m_numberOfClients),
                        MakeUintegerChecker<uint16_t>())

          // 添加 ClientId 属性，用于标识单个客户端
          .AddAttribute("ClientId", "The ID of the this client object, for logging purposes", UintegerValue(0),
                        MakeUintegerAccessor(&TcpStreamClient::m_clientId), MakeUintegerChecker<uint32_t>());
...
int TcpStreamClient::ReadInBitrateValues(std::string segmentSizeFile) {
  ...
  while (std::getline(myfile, temp)) {
    ...
    m_videoData.averageBitrate.push_back((8.0 * averageByteSizeTemp) /                // 字节转比特
...
void TcpStreamClient::SegmentReceivedHandle() {
  ...
  if (m_segmentCounter > 0) {
    ...
    m_bufferData.bufferLevelOld.push_back(std::max(
        m_bufferData.bufferLevelNew.back() - (m_transmissionEndReceivingSegment - m_throughput.transmissionEnd.back()),
        (int64_t)0));
  } else {
    // 如果是第一段，旧缓冲量为0
  ...
  // 记录吞吐量相关信息
  m_throughput.bytesReceived.push_back(m_videoData.segmentSize.at(m_currentRepIndex).at(m_segmentCounter));  // 本段大小
  m_throughput.transmissionStart.push_back(m_transmissionStartReceivingSegment);  // 接收开始时间
  m_throughput.transmissionRequested.push_back(m_downloadRequestSent);            // 请求下载时间
  m_throughput.transmissionEnd.push_back(m_transmissionEndReceivingSegment);      // 接收结束时间
...
void TcpStreamClient::StartApplication(void) {
  ...
  if (m_socket == 0) {
    ...
    // 设置连接成功/失败回调
    m_socket->SetConnectCallback(MakeCallback(&TcpStreamClient::ConnectionSucceeded, this),
...
void TcpStreamClient::StopApplication() {
  ...
  if (m_socket != 0) {
    ...
    m_socket->SetRecvCallback(MakeNullCallback<void, Ptr<Socket>>());  // 清空回调
...
void TcpStreamClient::LogDownload() {
  NS_LOG_FUNCTION(this);
  downloadLog << std::setfill(' ') << std::setw(13) << m_segmentCounter << " " << std::setfill(' ') << std::setw(21)
              << m_downloadRequestSent / (double)1000000 << " " << std::setfill(' ') << std::setw(14)
              << m_transmissionStartReceivingSegment / (double)1000000 << " " << std::setfill(' ') << std::setw(12)
              << m_transmissionEndReceivingSegment / (double)1000000 << " " << std::setfill(' ') << std::setw(12)
              << m_videoData.segmentSize.at(m_currentRepIndex).at(m_segmentCounter) << " " << std::setfill(' ')
              << std::setw(12) << "Y\n";
  downloadLog.flush();
}
...
void TcpStreamClient::LogAdaptation(algorithmReply answer) {
  NS_LOG_FUNCTION(this);
  adaptationLog << std::setfill(' ') << std::setw(13) << m_segmentCounter << " " << std::setfill(' ') << std::setw(9)
                << m_currentRepIndex << " " << std::setfill(' ') << std::setw(22)
                << answer.decisionTime / (double)1000000 << " " << std::setfill(' ') << std::setw(4)
                << answer.decisionCase << " " << std::setfill(' ') << std::setw(9) << answer.delayDecisionCase << "\n";
  adaptationLog.flush();
}
...
void TcpStreamClient::LogPlayback() {
  NS_LOG_FUNCTION(this);
  playbackLog << std::setfill(' ') << std::setw(13) << m_currentPlaybackIndex << " " << std::setfill(' ')
              << std::setw(14) << Simulator::Now().GetMicroSeconds() / (double)1000000 << " " << std::setfill(' ')
              << std::setw(13) << m_playbackData.playbackIndex.at(m_currentPlaybackIndex) << "\n";
  playbackLog.flush();
}
...
void TcpStreamClient::InitializeLogFiles(std::string simulationId, std::string clientId, std::string numberOfClients) {
  ...
  // 构造下载日志文件路径并打开
  std::string dLog = dashLogDirectory + m_algoName + "/" + numberOfClients + "/sim" + simulationId + "_" + "cl" +
                     clientId + "_" + "downloadLog.txt";
  ...
  // 构造播放日志文件路径并打开
  std::string pLog = dashLogDirectory + m_algoName + "/" + numberOfClients + "/sim" + simulationId + "_" + "cl" +
                     clientId + "_" + "playbackLog.txt";
  playbackLog.open(pLog.c_str());
  playbackLog << "Segment_Index Playback_Start Quality_Level\n";
  playbackLog.flush();

  // 构造自适应日志文件路径并打开
  std::string aLog = dashLogDirectory + m_algoName + "/" + numberOfClients + "/sim" + simulationId + "_" + "cl" +
                     clientId + "_" + "adaptationLog.txt";
  adaptationLog.open(aLog.c_str());
  adaptationLog << "Segment_Index Rep_Level Decision_Point_Of_Time Case DelayCase\n";
  adaptationLog.flush();

  // 构造缓冲日志文件路径并打开
  std::string bLog = dashLogDirectory + m_algoName + "/" + numberOfClients + "/sim" + simulationId + "_" + "cl" +
                     clientId + "_" + "bufferLog.txt";
  bufferLog.open(bLog.c_str());
  bufferLog << "     Time_Now  Buffer_Level \n";
  bufferLog.flush();

  // 构造吞吐量日志文件路径并打开
  std::string tLog = dashLogDirectory + m_algoName + "/" + numberOfClients + "/sim" + simulationId + "_" + "cl" +
                     clientId + "_" + "throughputLog.txt";
  throughputLog.open(tLog.c_str());
  throughputLog << "     Time_Now Bytes Received \n";
  throughputLog.flush();

  // 构造缓冲不足日志文件路径并打开
  std::string buLog = dashLogDirectory + m_algoName + "/" + numberOfClients + "/sim" + simulationId + "_" + "cl" +
                      clientId + "_" + "bufferUnderrunLog.txt";
