...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  Ptr<Node> pgw = epcHelper->GetPgwNode();  // PGW 节点（EPC 的出口）
  ...
  // ---------- 静态路由：让 R2 能把到 UE 的流量发给 PGW（7.0.0.0/8 是 EPC 默认
  // UE 网段） ----------
  ...
  // 同时给 PGW 加一条返回服务器网段的路由（通常 PGW 已有直连接口，这里做保险）
  Ptr<Ipv4StaticRouting> pgwStatic =
      ipv4RoutingHelper.GetStaticRouting(pgw->GetObject<Ipv4>());
  pgwStatic->AddNetworkRouteTo(Ipv4Address("10.1.0.0"), Ipv4Mask("255.255.0.0"),
