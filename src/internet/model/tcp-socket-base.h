...
class TcpSocketBase : public TcpSocket {
 public:
  ...
  /**
   *  新添加的OWD相关的回调函数，参数包括OWD、序列号、包大小和是否重传
   *  这个回调函数可以用来记录每个数据包的OWD信息
   */
  TracedCallback<Time, SequenceNumber32, uint32_t, bool> m_owdTrace;
   ...
   */
  void
  UpdateNextTxSequence(SequenceNumber32 oldValue, SequenceNumber32 newValue);
   ...
   * \param paceWindow Boolean to enable or disable pacing of the initial
   * window
   */
  ...
  Connect(const Address&
              address);  // Setup endpoint and call ProcessAction() to connect
  virtual int Listen(void);  // Verify the socket is in a correct state and
                             // call ProcessAction() to listen
  virtual int Close(void);   // Close by app: Kill socket upon tx buffer emptied
  virtual int ShutdownSend(
      void);  // Assert the m_shutdownSend flag to prevent send to network
  virtual int ShutdownRecv(
      void);  // Assert the m_shutdownRecv flag to prevent forward to app
  virtual int
  Send(Ptr<Packet> p, uint32_t flags);  // Call by app to send data to network
  virtual int SendTo(
      Ptr<Packet> p, uint32_t flags,
  ...
  RecvFrom(uint32_t maxSize, uint32_t flags,
           Address& fromAddress);  // ... and write the remote address at
                                   // fromAddress
  virtual uint32_t GetTxAvailable(void) const;  // Available Tx buffer size
  virtual uint32_t GetRxAvailable(
      void) const;  // Available-to-read data size, i.e. value of m_rxAvailable
  virtual int
 ...
 protected:
  ...
  /**
   * \brief Configure the endpoint v6 to a local address. Called by Connect()
   * if Bind() didn't specify one.
   *
   * \returns 0 on success
   */
   ...
   */
  virtual void
  CompleteFork(Ptr<Packet> p, const TcpHeader& tcpHeader,
   ...
   */
  bool
  IsValidTcpSegment(const SequenceNumber32 seq, const uint32_t tcpHeaderSize,
  ...
  /**
   * \brief Called by the L3 protocol when it received an ICMP packet to pass
   * on to TCP.
   *
  ...
  /**
   * \brief Extract at most maxSize bytes from the TxBuffer at sequence seq,
   * add the TCP header, and send to TcpL4Protocol
   *
   ...
   *
   * This function is mimicking the role of tcp_rcv_established() in
   * tcp_input.c in Linux kernel.
   *
   ...
   * to the header.
   *
   * \param header TcpHeader where the method should add the window scale
   * option
   */
