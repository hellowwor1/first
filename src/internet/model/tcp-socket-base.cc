  ...
  static TypeId tid =
      TypeId("ns3::TcpSocketBase")
          ...
          .AddTraceSource("EcnCwrSeq", "Sequence of last received CWR",
                          ...
                          "ns3::SequenceNumber32TracedValueCallback")
          .AddTraceSource("Owd",
                          "TCP one-way delay measured per received TCP segment",
                          MakeTraceSourceAccessor(&TcpSocketBase::m_owdTrace),
                          "ns3::TracedCallback::Time,ns3::SequenceNumber32,ns3:"
                          ":uint32_t,ns3::bool");
  ...
  switch (m_state) {
    ...
    case CLOSED:
      ...
      if ((tcpHeader.GetFlags() & ~(TcpHeader::PSH | TcpHeader::URG)) !=
          TcpHeader::RST) {  // Since m_endPoint is not configured yet, we
                             // cannot use SendRST here
        TcpHeader h;
...
void TcpSocketBase::ProcessSynRcvd(Ptr<Packet> packet,
  ...
  if (tcpflags == 0 ||
    ...
    if (tcpHeader.GetSequenceNumber() ==
        m_tcb->m_rxBuffer
            ->NextRxSequence()) {  // In-sequence FIN before connection
...
void TcpSocketBase::ProcessClosing(Ptr<Packet> packet,
  ...
  if (tcpflags == TcpHeader::ACK) {
    ...
    if (tcpflags == TcpHeader::FIN ||
      ...
      SendEmptyPacket(TcpHeader::ACK);
    } else if (tcpflags != TcpHeader::RST) {  // Receive of SYN or SYN+ACK or
                                              // bad flags or pure data
      NS_LOG_LOGIC("Illegal flag " << TcpHeader::FlagsToString(tcpflags)
                                   << " received. Reset packet is sent.");
      SendRST();
    }
...
uint32_t TcpSocketBase::SendDataPacket(SequenceNumber32 seq, uint32_t maxSize,
  ...
  AddSocketTags(p);
  // 在这个位置打上计算OWD的标签，计算OWD需要知道这个包的发送时间和序列号
  TcpOwdTag owdTag;
  owdTag.SetTxTime(Simulator::Now());
  owdTag.SetTxSeq(seq);
  owdTag.SetPayloadSize(sz);
  owdTag.SetRetransmission(isRetransmission);
  p->AddPacketTag(owdTag);
  ...
  if (isRetransmission ==
    ...
    for (std::deque<RttHistory>::iterator i = m_history.begin();
      ...
      if ((seq >= i->seq) &&
        ...
        i->retx = true;
        i->count = ((seq + SequenceNumber32(sz)) -
                    i->seq);  // And update count in hist
        break;
      }
    }
  }
...
void TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader& tcpHeader) {
  ...
  // 在这里处理接收OWD信息
  TcpOwdTag owdTag;
  if (p->PeekPacketTag(owdTag)) {
    Time owd = Simulator::Now() - owdTag.GetTxTime();

    // NS_LOG_UNCOND("TCP OWD: rxSeq=" << tcpHeader.GetSequenceNumber()
    //                                 << " txSeq=" << owdTag.GetTxSeq()
    //                                 << " rxSize=" << p->GetSize()
    //                                 << " txPayload=" <<
    //                                 owdTag.GetPayloadSize()
    //                                 << " retrans=" <<
    //                                 owdTag.IsRetransmission()
    //                                 << " owd=" << owd.GetMilliSeconds()
    //                                 << " ms");
    m_owdTrace(owd, owdTag.GetTxSeq(), owdTag.GetPayloadSize(),
               owdTag.IsRetransmission());
  }
